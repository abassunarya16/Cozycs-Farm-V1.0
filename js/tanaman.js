...
var tanaman = (function() {
            ...
            htmlContent += `
                <div style="background: var(--inner-card-bg, #f9f9f9); border-left: 4px solid ${badgeColor}; border-radius: 8px; padding: 10px; font-size: 12px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                        <strong style="color: ${badgeColor};"><i class="far fa-calendar-alt"></i> ${h.tanggal || '-'} <span style="background:#2E7D32; color:#fff; padding:1px 6px; border-radius:4px; font-size:10px; margin-left:4px;">${h.hst || 0} HST</span></strong>
    ...
    function loadTable() {
        ...
        pageData.forEach(function(item) {
            ...
            var metricsHtml = '';
            if (valKat === 'Pruning') {
                metricsHtml = `<div><i class="fas fa-cut" style="color: #0288D1; width: 14px;"></i> <strong>${item.tipePruning || 'Rempes Cabang Air'}</strong></div>
                               <div style="margin-top: 3px;"><i class="fas fa-sitemap" style="color: #555; width: 14px;"></i> <strong>Ruas: ${item.ruasTarget || '-'}</strong></div>`;
            } else if (valKat === 'Polinasi') {
                metricsHtml = `<div><i class="fas fa-heart" style="color: #E65100; width: 14px;"></i> <strong>Ruas Kawin: ${item.posisiBunga || '-'}</strong></div>
                ...
                metricsHtml = `<div><i class="fas fa-ruler-vertical" style="color: #2E7D32; width: 14px;"></i> <strong>${item.tinggi || 0} ${t('unit_cm')} | ${item.daun || 0} ${t('unit_leaves')}</strong></div>
                               <div style="margin-top: 3px;"><i class="fas fa-arrows-alt-h" style="color: #0277BD; width: 14px;"></i> <strong>Ø ${item.batang || 0} ${t('unit_mm')}</strong></div>`;
            }
            ...
            html += `
                <div style="background: var(--card-bg, #fff); border: 1px solid var(--border-color, #e8e8e8); border-radius: 12px; padding: 14px; margin-bottom: 12px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 8px;">
                        ...
                        <div style="background: var(--inner-card-bg, #f9f9f9); padding: 10px; border-radius: 8px; min-height: 54px; display: flex; flex-direction: column; justify-content: center;">
                            ...
                            <div style="font-size: 12px; font-weight: bold; color: var(--text-color, #000); line-height: 1.4;">
                                ...
                                <div style="margin-top: 3px;"><i class="fas fa-th" style="color: #0277BD; width: 14px;"></i> <strong>${valGh} - ${valTalang}</strong></div>
                            </div>
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed var(--border-color, #eee); padding-top: 8px; margin-top: 4px;">
                        <span onclick="tanaman.showHistoryModal('${valTalang}', '${valGh}')" title="Lihat Timeline Rekam Jejak" style="cursor: pointer; color: #0277BD; font-size: 13px; padding: 4px; font-weight: bold;">
